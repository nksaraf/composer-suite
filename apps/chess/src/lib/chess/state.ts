...
export function loadPgn(
  ...
  const tokens = ms.trim().split(new RegExp(/\s+/)).join(",").replace(/,,+/g, ",").split(",");
...
export function putPiece(
  ...
  const sq = SQUARES[square];
  if (type === KING && state.kings[color] !== EMPTY && state.kings[color] !== sq) {
    return null;
  }
...
export function generateMoves(
  ...
  const add_move = (board: Board, moves: HexMove[], from: number, to: number, flags: number) => {
    ...
    if (piece && piece.type === PAWN && (rank(to) === RANK_8 || rank(to) === RANK_1)) {
...
export function piecesEliminated(state: State, color: Color): { [k in PieceSymbol]: number } {
  const pieces: Piece[] = [];
  ...
  return {
    [PAWN]: 8 - pieces.filter(p => p.type === PAWN).length,
    [QUEEN]: 1 - pieces.filter(p => p.type === QUEEN).length,
    [ROOK]: 2 - pieces.filter(p => p.type === ROOK).length,
    [BISHOP]: 2 - pieces.filter(p => p.type === BISHOP).length,
    [KNIGHT]: 2 - pieces.filter(p => p.type === KNIGHT).length
  } as { [k in PieceSymbol]: number };
}
...
export function makeMove(prevState: State, move: HexMove): State {
  ...
  if (state.castling[us]) {
    for (let i = 0, len = ROOKS[us].length; i < len; i++) {
      if (move.from === ROOKS[us][i].square && state.castling[us] & ROOKS[us][i].flag) {
  ...
  if (state.castling[them]) {
    for (let i = 0, len = ROOKS[them].length; i < len; i++) {
      if (move.to === ROOKS[them][i].square && state.castling[them] & ROOKS[them][i].flag) {
  ...
  if (typeof move === "string") {
    ...
    for (const moveObj of moves) {
      if (
        ...
        (!checkPromotion || !("promotion" in moveObj) || move.promotion === moveObj.promotion)
      ) {
        return moveObj;
      }
    }
